...
class TestConfig(TestCase):
    ...
    main_file = os.path.join(os.getcwd(), 'config.json')
    alt_file = os.path.join(os.getcwd(), 'config_alt.json')
    ...
    def test__load_config_file_load_order_prefer_env_over_local(self):
        config_file = os.path.join(os.getcwd(), 'config_alt.json')
        os.environ['hale_config'] = config_file
        Config.CONFIG = {}
        config = Config()
        self.assertEqual(config_file, config.CONFIG_FILE)
